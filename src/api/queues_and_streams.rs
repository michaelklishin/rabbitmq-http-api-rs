    ...
    P: Display,
{
    ...
    ///
    /// Requires the `management` user tag and have `read` permissions on the vhost. Does not modify state.
    pub async fn list_queues_in_paged(
        &self,
        virtual_host: &str,
        params: &PaginationParams,
    ) -> Result<Vec<responses::QueueInfo>> {
    ...
    /// by purging.
    ///
    /// Requires the `management` user tag and have `read` permissions on the vhost.
    ...
    ///
    /// Requires the `management` user tag and have `configure` permissions on the vhost.
    pub async fn declare_quorum_queue(&self, vhost: &str, name: &str) -> Result<()> {
