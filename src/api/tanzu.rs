    ...
    P: std::fmt::Display,
{
    ...
    pub async fn schema_definition_sync_status(&self) -> Result<SchemaDefinitionSyncStatus> {
        let response = self.http_get("definitions/sync/status", None, None).await?;
    ...
    pub async fn enable_schema_definition_sync_on_node(&self, node: Option<&str>) -> Result<()> {
        ...
        let _response = self.http_post_without_body(path, None, None).await?;
        Ok(())
    }
