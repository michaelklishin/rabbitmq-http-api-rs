    ...
    P: fmt::Display,
{
    ...
    ///
    /// The endpoint should include the protocol, host, port, and path to the API.
    /// For example: `http://localhost:15672/api` or `https://rabbitmq.example.com:15672/api`.
    /// The default endpoint is `http://localhost:15672/api`.
    ...
    ///
    /// This allows you to configure custom timeouts, proxy settings, TLS configuration,
    /// or other HTTP client behavior by providing your own configured `HttpClient` instance.
    ...
    P: fmt::Display,
{
    ...
    /// Returns detailed information about a specific AMQP connection.
    ///
    /// The connection name is typically generated by RabbitMQ and can be found
    /// in the output of `list_connections()`. This function provides comprehensive
    /// information including connection state, client properties, and channel count.
    pub fn get_connection_info(&self, name: &str) -> Result<responses::Connection> {
        self.get_api_request(path!("connections", name))
    }

    /// Returns detailed information about a specific RabbitMQ Stream connection.
    ///
    /// Stream connections are used by clients connecting via the RabbitMQ Stream protocol.
    /// This function provides information about the connection state, client details,
    /// and associated publishers/consumers.
    ...
    /// Forcefully closes a client connection with an optional reason message.
    ///
    /// This operation will immediately terminate the specified connection. The optional
    /// reason will be sent to the client before closing. Use this carefully as it will
    /// disrupt any ongoing operations on that connection.
    ...
    /// Forcefully closes all connections belonging to a specific user.
    ///
    /// This operation will terminate all active connections for the specified user,
    /// regardless of which virtual host they're connected to. An optional reason
    /// message can be provided. This is useful for administrative purposes when
    /// you need to disconnect a specific user from the cluster.
    ...
    /// Revokes all permissions for a user on a virtual host.
    ///
    /// This function removes all configure, write, and read permissions for the
    /// specified user on the given virtual host. The user will no longer be able
    /// to access resources in that virtual host until new permissions are granted.
    ...
    ///
    /// Creates a new queue or ensures an existing queue has the specified configuration.
    /// If the queue already exists with different parameters, this operation may fail
    /// unless the parameters are compatible. Queue parameters include durability,
    /// auto-delete behavior, arguments, and other queue-specific settings.
    ...
    /// Declares a RabbitMQ stream with the specified parameters.
    ///
    /// Streams are a durable, replicated data structure in RabbitMQ designed for
    /// high-throughput scenarios. Unlike traditional queues, streams are append-only
    /// logs that support multiple consumers reading from different offsets. This function
    /// creates a new stream or ensures an existing stream has the specified configuration.
    ...
    ///
    /// Creates a new exchange or ensures an existing exchange has the specified configuration.
    /// Exchanges route messages to queues based on routing rules. The exchange type
    /// (direct, topic, fanout, headers) determines the routing behavior. If the exchange
    /// already exists with incompatible parameters, this operation may fail.
    ...
    /// The routing key and arguments specify the conditions under which messages
    /// will be delivered to the queue. The meaning of the routing key depends on
    ...
    /// Creates a binding between two exchanges (exchange-to-exchange binding).
    ///
    /// This allows messages published to the source exchange to be forwarded to
    /// the destination exchange based on the routing key and binding arguments.
    ...
    /// Lists all Shovel instances across the cluster.
    ///
    /// Shovels are a RabbitMQ plugin that moves messages between brokers or between
    /// different locations within the same broker. This function returns information
    /// about all configured Shovel instances, including their status, configuration,
