...
fn arb_vhost_name() -> impl Strategy<Value = String> {
    prop::string::string_regex(
        r"rust\.tests\.blocking\.proptest\.definitions\.vhosts\.[a-zA-Z0-9_-]{8,20}",
    )
    .unwrap()
}
...
fn arb_username() -> impl Strategy<Value = String> {
    prop::string::string_regex(
        r"rust\.tests\.blocking\.proptest\.definitions\.users\.[a-zA-Z0-9_-]{8,20}",
    )
    .unwrap()
}
...
fn arb_queue_name() -> impl Strategy<Value = String> {
    prop::string::string_regex(
        r"rust\.tests\.blocking\.proptest\.definitions\.queues\.[a-zA-Z0-9_-]{8,20}",
    )
    .unwrap()
}
...
fn arb_exchange_name() -> impl Strategy<Value = String> {
    prop::string::string_regex(
        r"rust\.tests\.blocking\.proptest\.definitions\.exchanges\.[a-zA-Z0-9_-]{8,20}",
    )
    .unwrap()
}
...
fn arb_policy_name() -> impl Strategy<Value = String> {
    prop::string::string_regex(
        r"rust\.tests\.blocking\.proptest\.definitions\.policies\.[a-zA-Z0-9_-]{8,20}",
    )
    .unwrap()
}
...
proptest! {
    ...
    #[test]
    fn prop_blocking_export_import_cluster_definitions_roundtrip(
        ...
        let _ = client.delete_vhost(&vhost_name, true);

        let vh_params = VirtualHostParams::named(&vhost_name);
        let create_vhost_result = client.create_vhost(&vh_params);
        ...
        let declare_exchange_result = client.declare_exchange(&vhost_name, &xp);
        ...
        let declare_queue_result = client.declare_queue(&vhost_name, &qp);
        prop_assert!(declare_queue_result.is_ok(), "Failed to declare a queue: {declare_queue_result:?}");

        await_metric_emission(20);

        let bind_result = client.bind_queue(&vhost_name, &queue_name, &exchange_name, None, None);
        prop_assert!(bind_result.is_ok(), "Failed to bind a queue: {bind_result:?}");

        await_metric_emission(100);

        let export_result = client.export_cluster_wide_definitions_as_data();
        ...
        prop_assert!(queue_found, "Queue not found in exported definitions");

        let exchange_found = exported_defs.exchanges.iter().any(|x| x.name == exchange_name && x.vhost == vhost_name);
        prop_assert!(exchange_found, "Exchange not found in exported definitions");
        ...
        let import_vhost_result = client.create_vhost(&vh_params);
        ...
        let import_result = client.import_vhost_definitions(&vhost_name, vhost_def);
        prop_assert!(import_result.is_ok(), "Failed to import the virtual host definitions: {import_result:?}");

        await_metric_emission(100);

        let get_queue_result = client.get_queue_info(&vhost_name, &queue_name);
        prop_assert!(get_queue_result.is_ok(), "Imported queue was not found: {get_queue_result:?}");

        let get_exchange_result = client.get_exchange_info(&vhost_name, &exchange_name);
    ...
    #[test]
    fn prop_blocking_export_import_vhost_definitions_with_users_and_permissions(
        vhost_name in arb_vhost_name(),
        ...
        let create_user_result = client.create_user(&user_params);
        ...
        let set_permissions_result = client.declare_permissions(&permissions);
        ...
        let import_result = client.import_cluster_wide_definitions(cluster_def);
        prop_assert!(import_result.is_ok(), "Failed to import the cluster definitions: {import_result:?}");

        let get_user_result = client.get_user(&username);
        ...
        let get_permissions_result = client.get_permissions(&vhost_name, &username);
    ...
    #[test]
    fn prop_blocking_export_import_definitions_with_policies(
        vhost_name in arb_vhost_name(),
        ...
        let declare_policy_result = client.declare_policy(&policy_params);
        prop_assert!(declare_policy_result.is_ok(), "Failed to declare a policy: {declare_policy_result:?}");

        let export_result = client.export_vhost_definitions_as_data(&vhost_name);
        ...
        let _ = client.delete_policy(&vhost_name, &policy_name);
        ...
        let get_policy_result = client.get_policy(&vhost_name, &policy_name);
    ...
    #[test]
    fn prop_blocking_definition_consistency_across_formats(
        vhost_name in arb_vhost_name()
    ) {
        let endpoint = endpoint();
        let client = Client::new(&endpoint, USERNAME, PASSWORD);
        ...
        let export_string_result = client.export_vhost_definitions_as_string(&vhost_name);
        prop_assert!(export_string_result.is_ok(), "Failed to export as a string: {export_string_result:?}");

        let export_data_result = client.export_vhost_definitions_as_data(&vhost_name);
    ...
    #[test]
    fn prop_blocking_policy_target_conversion_and_matching(
        vhost_name in arb_vhost_name(),
        policy_name in arb_policy_name(),
        queue_name in arb_queue_name(),
        exchange_name in arb_exchange_name(),
        ...
        let pattern = "rust\\.tests\\.blocking\\.proptest\\.definitions\\..*";
        let policy_params = PolicyParams {
            vhost: &vhost_name,
            name: &policy_name,
            pattern,
        ...
        if queue_type_matches {
            ...
            let list_result = client.list_queues_in(&vhost_name);
